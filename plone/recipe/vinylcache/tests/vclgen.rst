...
Create a config object::
    ...
    >>> config = {}
    >>> VclGenerator(config)
    <plone.recipe.vinylcache.vclgen.VclGenerator object at 0x...>
...
The rendered VCL actually uses the `path` (unix socket) form for this backend,
since it always sets `.path` when a backend dict has a `path` key::

    >>> '.path = "/tmp/foo.sock";' in result
    True
...
The ``cookiepassnotexclude`` regexp lets a URL opt out of the cookie-pass
exclude rules (e.g. so ``++resource++zmi`` keeps requiring auth even for
js/css). The rendered VCL must include the check against this regexp
inside the cookie-pass block::

    >>> config['cookiepassnotexclude'] = '/++resource++zmi/'
    >>> result = VclGenerator(config)()
    >>> 'req.url !~ "/++resource++zmi/"' in result
    True
