...
# Neutral name on purpose: this recipe targets exactly one upstream version
# at a time, so the template file itself should not need renaming the next
# time the supported Varnish/Vinyl Cache version changes.
VCL_TEMPLATE = "varnish.vcl.jinja2"
...
class VclGenerator(object):
    ...
    def __call__(self):
        ...
        # VCL syntax version 4.1 is what's generated; kept as a string (not
        # a float) so template comparisons like `vcl_version == '4.1'` work.
        data["vcl_version"] = "4.1"
        # render vcl file
